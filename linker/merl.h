...
struct Entry {
  ...
  // Address to be relocated for REL entries, or label location for ESD/ESR
  // entries.
  int location;
  ...
  // This is NOT the same as writing the entry out as raw binary data. This
  // function is provided for debugging purposes only. Use operator<< to write
  // the binary representation of the entry.
  ...
  // Throws std::runtime_error if the first word in the entry is not a valid
  // entry type, that is, if it is something other than 0x1 (REL), 0x5 (ESD), or
  // 0x11 (ESR).
...
struct MERL {
  ...
  // This is the size in bytes of the code segment, plus twelve (the size in
  // bytes of the header). This is the third word in the MERL header.
  int endCode = 0;
  ...
  // Writes a human-readable representation of the MERL file to the given output
  // stream. This is NOT the same as writing the MERL file out as raw binary
  // data. This function is provided for debugging purposes only. Use operator<<
  // to write the MERL file as binary.
  void print(std::ostream& os) const;
  ...
  // - The file ends prematurely, before the number of bytes indicated by the
  // second word read
  //   (which is expected to be the total size of the file).
  // - The footer contains an invalid entry.
  MERL(std::istream& is);

  // "Linking constructor" which constructs a new MERL file by linking the two
  // given MERL files. You are expected to implement this constructor. An
  // implementation is not provided by merl.o. This constructor is allowed to
  // modify the given MERL files m1 and m2.
